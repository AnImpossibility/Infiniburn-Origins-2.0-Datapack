scoreboard objectives add temp dummy
scoreboard objectives add temp2 dummy
scoreboard objectives add temp3 dummy

scoreboard objectives add raycast_completed dummy
scoreboard objectives add selection_completed dummy
scoreboard objectives add tp_ready dummy

scoreboard objectives add should_finish dummy
scoreboard objectives add ray_steps dummy

scoreboard objectives add enderian_focus dummy

scoreboard objectives add tp_config dummy
scoreboard players set minDelta tp_config -1
scoreboard players set maxDelta tp_config 2